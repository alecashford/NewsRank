require 'spec_helper'
describe GetScores::TwitterFetcher do
  describe '#fetch' do
  ...
  describe '#count' do
    it "returns the correct count from Twitter" do
      stub_request(:get, "http://cdn.api.twitter.com/1/urls/count.json?url=http://www.brushnewstribune.com/ci_26167090/council-receives-request-revisit-marijuana-moratorium").to_return(status: 200, body: '{"count":18}', headers: {:content_type => 'application/json'})
      client = GetScores::TwitterFetcher.new("http://www.brushnewstribune.com/ci_26167090/council-receives-request-revisit-marijuana-moratorium")
      expect(client.count).to eq(18)
    end

    it "returns nil if any error from Twitter API" do
      stub_request(:get, "http://cdn.api.twitter.com/1/urls/count.json?url=http://www.brushnewstribune.com/ci_26167090/council-receives-request-revisit-marijuana-moratorium").to_raise(StandardError.new("any error"))
      client = GetScores::TwitterFetcher.new("http://www.brushnewstribune.com/ci_26167090/council-receives-request-revisit-marijuana-moratorium")
      expect(client.count).to eq(nil)
    end
  end
end